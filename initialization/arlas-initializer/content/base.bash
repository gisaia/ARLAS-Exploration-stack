...
arlas (){
  ...
  curl -s -X PUT --header 'Content-Type: application/json;charset=utf-8' --header 'Accept: application/json' -d@/initialization/server_collection.json "$server_URL_for_initializer/arlas/collections/$server_collection_name"
  echo
}

load_WUI_configuration () {
  log "Loading WUI configuration..."
  jq -r ".arlas.server.url=\"$server_URL_for_client/arlas\" | .arlas.server.collection.name=\"$server_collection_name\"" "/initialization/WUI_configuration.json" > /wui-configuration/config.json
