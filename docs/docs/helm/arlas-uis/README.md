...
| authent.forceConnect | bool | `false` | When authentication is enabled, this option forces to be connected to Identity Provider at application bootstrap |
...
| authent.silentRefreshTimeout | string | `"10000"` | Timeout for silent refresh |
...
| links | list | `[]` | List of links to be added in the left menu of the WUI. Each link must contain `icon`, `url` and `name` attributes. |
...
| uis.hub.image | string | `"gisaia/arlas-wui-hub:27.1.0-rc.1"` |  |
...
| uis.wui.image | string | `"gisaia/arlas-wui:27.1.0-rc.1"` |  |
