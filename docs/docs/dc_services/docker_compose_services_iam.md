...
Image: `ARLAS_PERSISTENCE_VERSION` with `gisaia/arlas-persistence-server:26.0.1` in `conf/versions.env`
...
Image: `ARLAS_IAM_SERVER_VERSION` with `gisaia/arlas-iam-server:26.0.3` in `conf/versions.env`
